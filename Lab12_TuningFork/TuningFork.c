...
void Sound_Init(void){ 
  ...
  NVIC_ST_RELOAD_R = 90908;       // reload value for 500us
...
void SysTick_Handler(void){
		...
		if(GPIO_PORTA_DATA_R&0x08 && down) {
			down=0;
			pressed=!pressed;
		}
	...
	if(pressed){    //while PA3 pressed 
	if(GPIO_PORTA_DATA_R&0x04){   // toggle PA5
    GPIO_PORTA_DATA_R &= ~0x04; // make PA5 low
    NVIC_ST_RELOAD_R = 90908;     // reload value for low phase
  } 
	else{
    GPIO_PORTA_DATA_R |= 0x04;  // make PA5 high
    NVIC_ST_RELOAD_R = 90908;     // reload value for high phase
  }
	}
}
